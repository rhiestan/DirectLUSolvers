...
class SupernodalLU : public SparseSolverBase<SupernodalLU<MatrixType_, OrderingType_, Executor_>> {
 ...
 private:
  ...
  // analysis helper. The rest of the symbolic pipeline -- elimination tree,
  // postorder, supernode partition, block structure, update lists, scheduling
  // levels -- depends on the pattern alone and lives in SupernodalLUSymbolic.h,
  // shared with this project's other supernodal solvers. Only the step that
  // reads THIS solver's matrix and numbering is a member.
