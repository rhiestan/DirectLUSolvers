// Faithful, bit-identical port of METIS's multiple minimum degree ordering
// (libmetis/mmd.c: genmmd/mmdelm/mmdint/mmdnum/mmdupd), used as the base case
// for small subgraphs at the bottom of the nested-dissection recursion
// (MMDSWITCH, ometis.c). Ported from a FORTRAN subroutine in SPARSPAK
// (George/Liu/Chu/Ng) via SMMS -- the original is 1-based, goto-heavy, and
// destroys adjncy in place; all three properties are preserved here
// deliberately rather than "modernized," because this is exactly the kind of
// dense, branch-heavy numerical code where a well-intentioned rewrite is most
// likely to silently change behaviour. No RNG involved anywhere in this file.
//
// Callers are responsible for the same two conventions the reference's own
// caller (MMDOrder, ometis.c) uses:
//   1. xadj/adjncy must already be 1-based *values* (every stored index
//      incremented by one) -- this module does not do that shift itself.
//   2. adjncy is destroyed by genmmd; do not reuse it afterward.
// Internally, genmmd's own 1-based *indexing* (xadj[1] is node 1, etc.) is
// implemented the same way the reference does it: decrementing the pointers
// once at entry so xadj[node] lines up for node in [1, neqns]. This mirrors
// libmetis/mmd.c's own `xadj--; adjncy--; ...` exactly.

#ifndef DIRECTLUSOLVERS_HEADER_ONLY_METIS_MINIMUM_DEGREE_H
#define DIRECTLUSOLVERS_HEADER_ONLY_METIS_MINIMUM_DEGREE_H
...
void mmdelm(IndexT mdeg_node, IndexT* xadj, IndexT* adjncy, IndexT* head, IndexT* forward,
           IndexT* backward, IndexT* qsize, IndexT* list, IndexT* marker, IndexT maxint, IndexT tag);

template <typename IndexT>
IndexT mmdint(IndexT neqns, IndexT* xadj, IndexT* adjncy, IndexT* head, IndexT* forward,
             IndexT* backward, IndexT* qsize, IndexT* list, IndexT* marker);

template <typename IndexT>
void mmdnum(IndexT neqns, IndexT* perm, IndexT* invp, IndexT* qsize);

template <typename IndexT>
void mmdupd(IndexT ehead, IndexT neqns, IndexT* xadj, IndexT* adjncy, IndexT delta, IndexT* mdeg,
           IndexT* head, IndexT* forward, IndexT* backward, IndexT* qsize, IndexT* list,
           IndexT* marker, IndexT maxint, IndexT* tag);
...
**************************************************************************/
template <typename IndexT>
void genmmd(IndexT neqns, IndexT* xadj, IndexT* adjncy, IndexT* invp, IndexT* perm, IndexT delta,
           IndexT* head, IndexT* qsize, IndexT* list, IndexT* marker, IndexT maxint, IndexT* ncsub) {
  IndexT ehead, i, mdeg, mdlmt, mdeg_node, nextmd, num, tag;
  ...
  *ncsub = 0;
  ...
  while (1) {
    ...
    mdlmt = std::min(neqns, mdeg + delta);
    ehead = 0;
  ...
  n900:
    ...
    if (num > neqns) goto n1000;
    mmdupd(ehead, neqns, xadj, adjncy, delta, &mdeg, head, invp, perm, qsize, list, marker, maxint,
          &tag);
  }; /* end of -- while ( 1 ) -- */
...
void mmdelm(IndexT mdeg_node, IndexT* xadj, IndexT* adjncy, IndexT* head, IndexT* forward,
           IndexT* backward, IndexT* qsize, IndexT* list, IndexT* marker, IndexT maxint, IndexT tag) {
  IndexT element, i, istop, istart, j, jstop, jstart, link, nabor, node, npv, nqnbrs, nxnode,
...
template <typename IndexT>
void mmdnum(IndexT neqns, IndexT* perm, IndexT* invp, IndexT* qsize) {
  IndexT father, nextf, node, nqsize, num, root;
  ...
  for (node = 1; node <= neqns; node++) {
    if (perm[node] <= 0) {
      ...
      father = node;
      while (perm[father] <= 0) father = -perm[father];
...
void mmdupd(IndexT ehead, IndexT neqns, IndexT* xadj, IndexT* adjncy, IndexT delta, IndexT* mdeg,
           IndexT* head, IndexT* forward, IndexT* backward, IndexT* qsize, IndexT* list,
           IndexT* marker, IndexT maxint, IndexT* tag) {
  IndexT deg, deg0, element, enode, fnode, i, iq2, istop, istart, j, jstop, jstart, link, mdeg0,
      mtag, nabor, node, q2head, qxhead;
...
#endif  // DIRECTLUSOLVERS_HEADER_ONLY_METIS_MINIMUM_DEGREE_H
